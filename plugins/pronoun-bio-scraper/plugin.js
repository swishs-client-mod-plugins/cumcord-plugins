...
* @license   GPL-3.0 GNU General Public License v3.0
* @link      https://github.com/swishs-client-mod-plugins/cumcord-plugins/pronoun-bio-scraper
*/const{getMessage:I}=s.findByProps("getMessages"),{fetchProfile:S}=s.findByProps("fetchProfile"),{getUserProfile:U}=s.findByProps("getUserProfile"),{getChannelId:T}=s.findByProps("getVoiceChannelId"),{openModal:O}=s.findByProps("openModal","openModalLazy"),{getUser:A}=s.findByProps("getUser","getCurrentUser"),M=s.findByProps("MenuGroup","MenuItem"),L=s.find(e=>e.default?.displayName==="MessageTimestamp"),_=s.find(e=>e.default?.displayName==="MessageContextMenu"),R=e=>{if(!e)return;e=e.replace(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/,"");let n=e.match(/[a-z]+?(?:\/[a-z]+)+/i);if(!n&&~e.indexOf("pronouns")){if(~e.indexOf("any"))return"any pronouns";if(~e.indexOf("avoid"))return"avoid pronouns, use my name";if(~e.indexOf("ask"))return"ask for pronouns"}if(n)return n[0]};let u=[],p=[],P=[];var F=()=>({onLoad(){u.push(f.after("default",L,([e],n)=>{let r=I(T(),e.id?.substring(18)),h=A(r?.author?.id),a=!U(r?.author?.id);a&&o.persist.ghost.fetch&&!P.includes(r?.author?.id)&&p.includes(r?.id)&&(P.push(r?.author?.id),S(r?.author?.id)),p.includes(r?.id)||p.push(r?.id);let c=r?.author?.id in o.persist?o.persist[r.author.id]:a?"still loading":R(h?.bio);!c||u.push(f.after("children",n.props.children.props,(y,i)=>{if(i.props.children.props.children[1]!=="edited")return i.props.children.props.children[1]+=` \u2022 ${c}`,i}))})),u.push(f.after("default",_,([e],n)=>(g.findInReactTree(n,r=>r?.props?.id=="pronouns")||n.props.children.push(t.React.createElement(M.MenuGroup,null,t.React.createElement(M.MenuItem,{id:"pronouns",label:"Overide Pronouns",action:()=>O(r=>t.React.createElement(z,{e:r,author:e.message.author.id,extract:R}))}))),n)))},onUnload(){u.forEach(e=>e())},settings:t.React.createElement(v,null)});return F})(cumcord.modules.webpack,cumcord.pluginData,cumcord.utils,cumcord.modules.common,cumcord.ui.components,cumcord.patcher);
